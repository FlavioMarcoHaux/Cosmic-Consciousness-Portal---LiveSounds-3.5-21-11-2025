...
export const getYouTubeSEO = async (theme: string, focus: string, language: 'pt' | 'en' = 'pt'): Promise<YouTubeSEO | null> => {
    const schema = {
        type: Type.OBJECT,
        properties: {
    ...
    return await generateJsonContent(Prompts.youtubeAgentPrompt(theme, focus, language), schema, fallback);
}

// --- Thumbnail Prompt Generation (New Separate Step) ---
export const getYouTubeThumbnailPrompt = async (title: string, theme: string, language: 'pt' | 'en' = 'pt'): Promise<string | null> => {
    try {
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash',
            contents: Prompts.thumbnailGenerationPrompt(title, theme, language),
        });
        return response.text;
    } catch (error) {
