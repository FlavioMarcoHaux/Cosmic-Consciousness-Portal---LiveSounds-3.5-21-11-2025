...
const DurationSelector: React.FC<{ selected: number; onSelect: (duration: number) => void; disabled?: boolean; }> = ({ selected, onSelect, disabled }) => (
    <div className="flex flex-wrap justify-center gap-2 mb-6">
        {DURATIONS.map(d => (
            <button key={d} onClick={() => onSelect(d)} disabled={disabled} className={`px-3 py-1 text-xs rounded-full transition-all duration-300 border ${selected === d ? 'border-purple-400 bg-purple-500/30 text-white shadow-[0_0_10px_rgba(168,85,247,0.4)]' : 'border-white/10 text-indigo-300/60 hover:bg-white/5 hover:text-indigo-200'} ${disabled ? 'opacity-30 cursor-not-allowed' : ''}`}>
                {d} min
            </button>
        ))}
    </div>
...
const ArchetypeActivation: React.FC<ArchetypeActivationProps> = ({ readingIndex, cardIndex, onClose }) => {
    ...
    useEffect(() => {
        if (activationData) {
            // If we have data but no blob, we need to regenerate audio or rely on text
            // Since we don't persist the blob, the user gets the text-based buffer mode unless they regenerate.
            // However, ArchetypeActivation is usually short, so buffer mode is fine for revisit.
            // For new activations (long), we use the blob flow.
            setStatus('active');
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
            <div className="absolute inset-0 bg-gray-900/90 backdrop-blur-xl transition-opacity duration-500 animate-fadeIn" onClick={handleClose} />
            <div className="relative w-full max-w-5xl h-full max-h-[600px] bg-black/40 border border-purple-500/20 rounded-2xl shadow-2xl shadow-purple-900/40 flex flex-col md:flex-row overflow-hidden animate-scaleIn">
                <button onClick={handleClose} className="absolute top-4 right-4 z-20 text-white/50 hover:text-white p-2 rounded-full hover:bg-white/10 transition-colors">
                ...
                <div className="w-full md:w-5/12 h-1/3 md:h-full bg-gradient-to-b from-purple-900/20 to-black/60 flex flex-col items-center justify-center p-6 relative overflow-hidden border-b md:border-b-0 md:border-r border-purple-500/10">
                    ...
                    <div className={`relative w-40 sm:w-56 transition-all duration-700 transform ${status === 'invoking' || status === 'generating' ? 'scale-110 animate-pulse' : 'scale-100'}`}>
                    ...
                    <h3 className="mt-6 text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-200 to-indigo-100 text-center z-10">{cardData.name}</h3>
                    <p className="text-indigo-300/60 text-sm font-serif italic mt-1 text-center z-10">{cardData.position}</p>
                </div>
                ...
                <div className="w-full md:w-7/12 h-2/3 md:h-full p-6 md:p-10 flex flex-col items-center justify-center overflow-y-auto">
                    {status === 'idle' && (
                        <div className="text-center animate-fadeIn max-w-md">
                            ...
                            <p className="text-indigo-200/70 mb-8 leading-relaxed">Você está prestes a invocar o arquétipo de <strong>{cardData.name}</strong>. Esta meditação guiada irá integrar essa energia em sua psique.</p>
                            <DurationSelector selected={duration} onSelect={setDuration} />
                            <button onClick={handleInvoke} className="group relative px-8 py-4 bg-purple-600 text-white rounded-full font-semibold tracking-wide overflow-hidden transition-all hover:scale-105 hover:shadow-[0_0_20px_rgba(147,51,234,0.5)]">
                                <span className="relative z-10 flex items-center gap-2"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>Invocar Arquétipo</span>
                    ...
                    {(status === 'invoking' || status === 'generating') && (
                        <div className="text-center animate-fadeIn">
                            ...
                            <p className="text-lg text-purple-200 font-medium animate-pulse">
                                {status === 'invoking' ? 'Sintonizando frequências...' : 'Ancorando áudio sagrado...'}
                            </p>
                            {status === 'generating' && (
                                 <div className="w-48 mt-4 h-1 bg-purple-900/50 rounded-full overflow-hidden mx-auto">
                                     <div className="h-full bg-indigo-400 transition-all duration-300" style={{width: `${progress}%`}}></div>
                                 </div>
                            )}
                            <p className="text-sm text-indigo-300/60 mt-2">{status === 'invoking' ? 'Canalizando a sabedoria cósmica' : `Materializando (${progress}%)`}</p>
                        </div>
                    )}
                    ...
                    {status === 'error' && (
                        <div className="text-center animate-fadeIn">
                            ...
                            <button onClick={handleInvoke} className="px-6 py-2 border border-red-500/50 text-red-300 rounded-full hover:bg-red-500/20 transition-colors">Tentar Novamente</button>
                        </div>
                    )}
                        ...
                        <div className="w-full animate-fadeIn flex flex-col h-full">
                            <div className="flex-grow overflow-y-auto pr-2 custom-scrollbar">
                                <div className="text-center mb-6">
                                    ...
                                    <h2 className="text-2xl md:text-3xl font-serif text-transparent bg-clip-text bg-gradient-to-r from-white via-purple-200 to-white italic leading-tight">"{activationData.mantra}"</h2>
                                </div>
                            ...
                            <div className="mt-4 pt-4 border-t border-white/5 text-center">
                                <button onClick={() => { reset(); setStatus('idle'); setDuration(5); }} className="text-xs text-indigo-300/50 hover:text-indigo-200 transition-colors">Reiniciar Ritual</button>
                            </div>
                        </div>
                    )}
                </div>
